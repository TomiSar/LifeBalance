...
public class BalanceOfLife extends Application {
    ...
    public void start(Stage stage) {
        ...
        layout.getChildren().addAll(presentationLbl, person.getNameLbl(), name, person.getHeightLbl(), heightInput,
                person.getWeightLbl(), weightInput, person.getBmiButton(), person.getClearButton(), person.getNormalBMIFormulaText(), bmiResultNormal, person.getNickTrefethenBMIFormulaText(),
                bmiResult, bmiResultLbl);

        person.getBmiButton().setOnAction((event) -> {
            //Convert height and weigth to double and calculate the Body mass index from formula
            //Normal BMI formula = weight(kg)/height(m)^2 and Nick Trefethen BMI formula = 1.3*weight(kg)/height(m)^2.5
            ...
            sb.append("\nHello ").append(name.getText()).append("! Your height is ").append(heightInput.getText()).append("cm and your weight is ").append(weightInput.getText()).append("kg\n");
            ...
            if (newFormulaBmi != 0) {
                ...
                bmiResultLbl.setText(sb.toString());
            }
        });

        //Clear all text from TextFields, labels and calculation label 
        person.getClearButton().setOnAction((event) -> {
            name.setText("");
            heightInput.setText("");
            weightInput.setText("");
            bmiResultNormal.setText("");
            bmiResult.setText("");
            bmiResultLbl.setText("");
        });
